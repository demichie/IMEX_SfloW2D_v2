...
for i in range(nx_points - 1, -1, -1):
    ...
    if X[0, i] > 21.5:
        ...
        Z[:, i] = Z[:, i+1] + \
            np.sin(np.deg2rad(35.0))*(1.0-0.25*(X[:, i]-17.5))*dx
...
for i in range(ny_cells):
    ...
    # pore pressure
    q0[-1, :] = (PRES + rho_m * grav * H_cent[i, :]) * q0[2, :]
...
if (plot_flag):
    ...
    # Plot the surface.  The triangles in parameter space determine which
    # x, y, z points are connected by an edge.
    ax.plot_trisurf(X_cent1d[idx].flatten(),
                    Y_cent1d[idx].flatten(),
                    W_cent1d[idx].flatten(),
                    triangles=tri.triangles,
                    edgecolor='none')

    ax.plot_surface(X_cent, Y_cent, W_cent, alpha=0.3)
    cset = ax.contour(X_cent,
                      Y_cent,
                      H_cent,
                      zdir='z',
                      offset=-5,
