...
%
% This script generate an .asc file with the initial thickness for the
% simulation, obtaining by subtracting from the original den an ellipsoidal
% niche.
...
% - horizontal semiaxis of the ellipsoid orthogonal to the direction from
%   P1 to P2
...
%
"""
from input_ellipsoid import DEM_folder
from input_ellipsoid import DEM_file
from input_ellipsoid import x1
from input_ellipsoid import y1
from input_ellipsoid import vol
from input_ellipsoid import mod_vel
from input_ellipsoid import aspect_ratio
import numpy as np
...
def compute_vol_new(DEM, dist, XS, YS, x1, y1, semi_axis, aspect_ratio,
                    cell_topo, n):

    indexes = np.where(
        (dist > semi_axis - cell_topo) * (dist < semi_axis + cell_topo))
    ...
    # compute the second point elevation by linearly interpolating the
    # DEM data
    X2 = np.zeros((1, 1))
    X2[0, 0] = x2
    ...
    # define the center of the ellipsoid 3D coordinates and the vertical
    # semi-axis
    if z1 > z2:
...
def compute_vol(X_new, Y_new, DEM, zc, a, b, c, n):
    ...
    Z_temp1 = zc - c * (np.maximum(
        np.zeros_like(X_new), 1.0 - (np.abs(X_new / a))**2 - np.abs(
            (Y_new / b))**2))**(1.0 / n)
    Z_temp2 = Z_temp1 * ((X_new/a)**2 + (Y_new/b)**2 < 1) + \
        1.e5 ** ((X_new/a)**2 + (Y_new/b)**2 >= 1)
...
def interp2Dgrids(xin, yin, Zin, Xout, Yout):
    ...
    # Indexes of the top-right corner of the cell containing the center of the
    # parent lobe
    ix1 = ix + 1
    iy1 = iy + 1
...
xs_topo = lx_topo+0.5*cell_topo + \
    np.linspace(0, (cols_topo-1)*cell_topo, int(cols_topo))
ys_topo = ly_topo+0.5*cell_topo + \
    np.linspace(0, (rows_topo-1)*cell_topo, int(rows_topo))
...
Z_new_temp, Z_ell_temp, vol0, zc, alpha_rad = compute_vol_new(
    DEM, dist, XS, YS, x1, y1, semi_axis0, aspect_ratio, cell_topo, 2.0)
...
if vol0 > vol:
    ...
    Z_new_temp, Z_ell_temp, vol2, zc, alpha_rad = compute_vol_new(
        DEM, dist, XS, YS, x1, y1, semi_axis2, aspect_ratio, cell_topo, 2.0)
...
if vol2 < vol:
    ...
    for i in range(20):
        ...
        semi_axis1 = semi_axis0 + \
            ((vol-vol0)/(vol2-vol0))**(1.0/3.0)*(semi_axis2-semi_axis0)
        print('semi_axis1', semi_axis1)

        Z_new, Z_ell, vol1, zc, alpha_rad = compute_vol_new(
            DEM, dist, XS, YS, x1, y1, semi_axis1, aspect_ratio, cell_topo,
            2.0)
...
np.savetxt(DEM_folder + 'new_dem.asc',
           np.flipud(Z_new),
           header=header,
           fmt='%1.5f',
           comments='')
...
np.savetxt(DEM_folder + 'init_volume.asc',
           np.flipud(DEM - Z_new),
           header=header,
           fmt='%1.5f',
           comments='')
...
np.savetxt(DEM_folder + 'ellipsoid.asc',
           np.flipud(Z_ell),
           header=header,
           fmt='%1.5f',
           comments='')
