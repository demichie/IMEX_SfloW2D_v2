...
CONTAINS
  ...
  REAL(wp) FUNCTION lambertws(p)
    ...
    !
    REAL(wp) :: p,P2,P3,P4,P5,P6,P7,P8,P9,P10,P11,P12,P13,P14,P15,P16,P17,P18,  &
         P19,P20

    parameter (P2=-0.33333333333333333333E0_wp)
    parameter (P3=0.15277777777777777778E0_wp)
    parameter (P4=-0.079629629629629629630E0_wp)
    parameter (P5=0.044502314814814814815E0_wp)
    parameter (P6=-0.025984714873603762493E0_wp)
    parameter (P7=0.015635632532333921223E0_wp)
    parameter (P8=-0.0096168920242994317068E0_wp)
    parameter (P9=0.0060145432529561178610E0_wp)
    parameter (P10=-0.0038112980348919992267E0_wp)
    parameter (P11=0.0024408779911439826659E0_wp)
    parameter (P12=-0.0015769303446867842539E0_wp)
    parameter (P13=0.0010262633205076071544E0_wp)
    parameter (P14=-0.00067206163115613620400E0_wp)
    parameter (P15=0.00044247306181462090993E0_wp)
    parameter (P16=-0.00029267722472962744485E0_wp)
    parameter (P17=0.00019438727605453931782E0_wp)
    parameter (P18=-0.00012957426685274881888E0_wp)
    ...
    ELSEIF (abs(p).lt.0.0766E0_wp) THEN

       lambertws=-1.E0_wp+p*(1.E0_wp+p*(P2+p*(P3+p*(P4+p*(P5+p*(P6+p*(P7+p*     &
            (P8+p*(P9+p*P10)))))))))

    ELSE

       lambertws=-1.E0_wp+p*(1.E0_wp+p*(P2+p*(P3+p*(P4+p*(P5+p*(P6+p*(P7+p*     &
            (P8+p*(P9+p*(P10+p*(P11+p*(P12+p*(P13+p*(P14+p*(P15+p*(P16+p*       &
